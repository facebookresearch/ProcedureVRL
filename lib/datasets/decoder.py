...
#     print(orig_duration,tb,frames_length,duration,target_fps,num_frames)
    if duration is None and orig_duration is not None:
        duration = orig_duration / tb
        frames_length = int(orig_duration * fps)
...
#         decode_all_video = False
#         start_idx, end_idx = get_start_end_idx(
#             int(frames_length * (end-start) / orig_duration),
#             sampling_rate * num_frames / target_fps * fps,
#             clip_idx,
#             num_clips,
#         )
        
#         start_idx += start * sampling_rate / target_fps * fps
#         end_idx = min(end_idx + start * sampling_rate / target_fps * fps, end * sampling_rate / target_fps * fps)
#         #end_idx = min(end_idx + start  * fps, end * fps)
#         #print(start,end,start * sampling_rate / target_fps * fps,start_idx ,end_idx)
#         timebase = duration / frames_length
#         video_start_pts = int(start_idx * timebase)
#         video_end_pts = int(end_idx * timebase)
#         start = None
#         end = None
    ...
    frames = None
#     if'mkv' in str(container):
#         print(container.streams.video,start,end)
    # If video stream was found, fetch video frames from the video.
    if container.streams.video:
        if start is None and end is None:
            #print('video_start_pts,video_end_pts,duration,tb,timebase,start_idx, end_idx,frames_length',video_start_pts,video_end_pts,duration,tb,timebase,start_idx, end_idx,frames_length)
...
#             print(video_frames)
        if h<=360:
            ...
            #frames = [frame.to_rgb().to_ndarray() for frame in video_frames]
            frames = [cv2.resize(frame.to_rgb().to_ndarray(), [int(w/h*360),360], interpolation = cv2.INTER_CUBIC) for frame in video_frames]
        frames = torch.as_tensor(np.stack(frames))
#         print(frames.shape)
#         if'mkv' in str(container):
#             print('mkv',frames.shape, start, end)  
#             if frames.shape[0] < abs(start-end)*32:
#                 print(container, container.duration)
...
def decode(
    ...
    if start == None and end==None:
        ...
        #print(frames.shape[0], start_idx, end_idx)
    else:
        start_idx = 0
        end_idx = frames.shape[0]
    #print(start,end)    
