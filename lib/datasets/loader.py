...
from lib.datasets.multigrid_helper import ShortCycleBatchSampler

from . import utils as utils
from .build import build_dataset

def kbv_collate(batch):
    """
    ...
    for key in extra_data[0].keys():
        if not 'step' in key:
            collated_extra_data[key] = default_collate([d[key] for d in extra_data])
        else:
            collated_extra_data[key] = torch.cat([d[key] for d in extra_data], dim = 0)
...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    if cfg.MULTIGRID.SHORT_CYCLE and split in ["train"] and not is_precise_bn:
        ...
        elif hasattr(cfg.MODEL, 'KBC') and cfg.MODEL.KBC:
            collate = kbv_collate 
        else:
            collate = None
        # Create a loader
