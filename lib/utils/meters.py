...
class TestMeter(object):
    ...
    def update_stats(self, preds, labels, clip_ids, video_name_list=None):
        """
        ...
        for ind in range(preds.shape[0]):
            ...
            # record video name for visualization purpose
            if self.video_names[vid_id] == '' and video_name_list is not None:
                self.video_names[vid_id] = video_name_list[int(clip_ids[ind])].split('/')[-1]#.split('.')[0]
...
class EPICTestMeter(object):
    ...
    def finalize_metrics(self, ks=(1, 5), compute_recall=False):
        """
        Calculate and log the final ensembled metrics.
        ks (tuple): list of top-k values for topk_accuracies. For example,
            ks = (1, 5) correspods to top-1 and top-5 accuracy.
        ...
        if compute_recall:
            # verb_topks = metrics.topk_mean_recalls(self.verb_video_preds, self.verb_video_labels, ks, num_cls=97)
            # noun_topks = metrics.topk_mean_recalls(self.noun_video_preds, self.noun_video_labels, ks, num_cls=300)
            # action_topks = metrics.multitask_topks_recalls(
            ...
            # Compute the recalls	
            action_topks, verb_topks, noun_topks = metrics.compute_action_recall(self.verb_video_preds.numpy(), self.noun_video_preds.numpy(), 
                                                   self.verb_video_labels.numpy(), self.noun_video_labels.numpy(), ks)
            ...
            for k, verb_topk in zip(ks, verb_topks):
                stats["verb_top{}_recall".format(k)] = "{:.{prec}f}".format(verb_topk, prec=2)
            for k, noun_topk in zip(ks, noun_topks):
                stats["noun_top{}_recall".format(k)] = "{:.{prec}f}".format(noun_topk, prec=2)
            for k, action_topk in zip(ks, action_topks):	
                stats["action_top{}_recall".format(k)] = "{:.{prec}f}".format(action_topk, prec=2)
            logging.log_json_stats(stats)
